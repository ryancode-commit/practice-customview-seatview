package com.ra.ticketingapps.customview
...
class SeatsView : View {
    ...
    private val seats : ArrayList<Seat> = arrayListOf(
        ...
        Seat(id = 9, name = "E1", isBooked = false),
        Seat(id = 10, name = "E2", isBooked = false),
    )
    ...
    private val tittlePaint = Paint(Paint.FAKE_BOLD_TEXT_FLAG)
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawText(text, (width/2F) - 197F, 100F, tittlePaint)
    }

    private fun drawSeat(canvas: Canvas?, seat: Seat) {

        //mengatur warna ketika dibooking
        if (seat.isBooked){
...
//        //Bagian bawah kursi
        ...
        //Mengembalikkan ke pengaturan sebelumnya
        canvas?.restore()
    }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        val heightRowFive =(halfOfHeight + 600F)..(halfOfHeight + 800F)
        ...
        if (event?.action == ACTION_DOWN) {
            when {
                ...
                event.x in widthColumnOne && event.y in heightRowFive -> booking(8)
                event.x in widthColumnTwo && event.y in heightRowFive -> booking(9)
            }
        }
        return true
    }
